...
AVAILABILITY VETO (NFL-specific — HARD):
- The news brief leads with QUARTERBACK STATUS. If a shortlisted game has a BACKUP QB starting
  (or the starter OUT/DOUBTFUL) and the pick leans on that team's normal strength, SKIP that
  pick — the ratings model cannot see the QB; you are the veto. A backup QB is the single
  biggest mispricing risk in this sport.
- Same veto for multiple key starters OUT (top receiver + tackle class of absences).
- QUESTIONABLE alone is not an automatic skip — judge whether the pick's case survives.
- Extreme weather in the brief (wind 15+ mph, heavy precipitation) argues against Overs — weigh it.
...
- Use data types a recreational bettor understands at a glance: player/team name, QB status, injury news, streak, record, H2H, points per game, turnovers, rushing/passing yards, home/road splits, division form, weather, win/loss form.
...
- Never say: model, projection, edge %, fair price, EV (expected value), point deviations, power rating, DVOA, EPA, "expected points", blend, shortlist, sigma, quant, "our system", process metrics, or how the pick was selected.
...
Output ONLY a JSON array (no markdown fences). Each object:
  date (YYYY-MM-DD ET), sport "americanfootball_nfl", league "NFL", home, away, match "Away @ Home",
  kickoff_et, market "Moneyline"|"Total"|"Asian Handicap", selection_struct (HOME|AWAY|OVER x|UNDER x|HOME ±x|AWAY ±x),
