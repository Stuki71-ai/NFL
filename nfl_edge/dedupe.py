"""Same-day + cross-product dedupe (US EDGE + NFL EDGE share GQ Sports Picks sheet).
...
def claim_key(p: dict[str, Any]) -> str:
    ...
    return "_".join(
        [
            ...
            gnorm(p.get("sport") or "americanfootball_nfl"),
            gnorm(p.get("home")),
            gnorm(p.get("away")),
            gnorm(p.get("market")),
            gnorm(sel),
...
def load_sent_keys(out_dir: Path, date_et: str) -> set[tuple[str, str, str, str, str]]:
    """Keys already delivered by this NFL host today (last_sent + run JSON)."""
...
def load_shared_claim_keys(timeout: int = 20) -> set[str]:
    """All claim keys already on the shared Picks sheet (US EDGE + NFL EDGE + any claim)."""
