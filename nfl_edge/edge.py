...
from nfl_edge.utils import american_from_decimal
...
def shortlist_brief(cands: list[dict[str, Any]]) -> str:
    ...
    for i, c in enumerate(cands, 1):
        ...
        lines.append(
            ...
            f"  Model margin/total: {c['meta'].get('exp_margin')} / {c['meta'].get('exp_total')}\n"
            f"  Point deviations home off/def {notes.get('home_off')}/{notes.get('home_def')} "
            f"away off/def {notes.get('away_off')}/{notes.get('away_def')} "
