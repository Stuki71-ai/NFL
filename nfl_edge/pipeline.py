...
from nfl_edge import brain, delivery, edge, model, news, ratings, slate
...
from nfl_edge.utils import safe_print
...
def run(
    *,
    ...
    try:
        ...
        print("[2] Ratings (point deviations, season-blended + shrunk carryover)…")
        t0 = time.perf_counter()
        ...
        print(
            f"    teams={len(table)} league PPG={league.get('ppg')} "
        ...
        for ev in events:
            ...
            if not pr["home"] or not pr["away"]:
                continue
            tal = ratings.build_game_talent(pr["home"], pr["away"], table, league)
        ...
        if dry_run or not deliver:
            ...
            # Prevents NFL from Whop'ing a pick another EDGE product already owns (and vice versa).
