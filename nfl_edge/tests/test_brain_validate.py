from nfl_edge.brain import SYS, _kickoff_ok, _validate_against_shortlist
...
def test_composer_rules_match_us_edge_bar():
    ...
    assert "power rating" in SYS and "DVOA" in SYS and "EPA" in SYS
    assert "recreational bettor" in SYS.lower() or "at a glance" in SYS.lower()
    # NFL-specific: composer is the availability veto with QB status first
    assert "AVAILABILITY VETO" in SYS
    assert "QUARTERBACK" in SYS or "BACKUP QB" in SYS


def test_composer_models_opus_then_gpt_sol():
    from pathlib import Path

    from nfl_edge import config
