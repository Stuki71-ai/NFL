#!/usr/bin/env python3
"""Grade picks from an NFL EDGE out/run_YYYY-MM-DD.json against NFL final scores (ESPN).
...
def espn_scores(date_et: str) -> list[dict]:
    ...
    ymd = date_et.replace("-", "")
    r = requests.get(
        f"https://site.api.espn.com/apis/site/v2/sports/football/nfl/scoreboard?dates={ymd}",
        timeout=25,
    )
    r.raise_for_status()
    games = []
...
def main() -> int:
    ...
    print(f"=== NFL EDGE GRADES — {args.date} ===")
    print(f"picks file: {path.name} | n={len(picks)}\n")

    sched = espn_scores(args.date)
    print(f"NFL games that day: {len(sched)}\n")
