#!/usr/bin/env python3
"""
NFL EDGE — always keep Git, this PC, and the VPS in lockstep.

Default env: C:\\Users\\istva\\.claude\\CODE\\.env  (override with NFL_EDGE_ENV)
...
HARD SCOPE (operator mandate — never violate):
  - ONLY /root/nfl-edge and NBA systemd units (nfl-edge-live.service/timer).
  - Strip only NFL EDGE lines from root crontab; never wipe other jobs.
...
VPS_REMOTE_DIR = "/root/nfl-edge"
# Files hashed on both sides for VERIFY
VERIFY_RELPATHS = [
    "nfl_edge/pipeline.py",
    "nfl_edge/brain.py",
    "nfl_edge/edge.py",
    "nfl_edge/config.py",
    "nfl_edge/delivery.py",
    "nfl_edge/dedupe.py",
    "nfl_edge/news.py",
    "nfl_edge/ratings.py",
    "nfl_edge/model.py",
    "nfl_edge/slate.py",
    "scripts/run_nfl_edge.py",
    "requirements.txt",
]
SECRET_KEYS = [
    "ODDS_API_KEY",
...
def deploy_vps(env: dict[str, str], install_cron: bool) -> None:
    ...
    setup = (
        ...
        f"\"from nfl_edge.pipeline import run; from nfl_edge.brain import SYS; "
    ...
    if install_cron:
        # HARD RULE: only NFL EDGE schedule units + strip our lines from root crontab.
        ...
        svc_local = ROOT / "deploy" / "nfl-edge-live.service"
        tim_local = ROOT / "deploy" / "nfl-edge-live.timer"
        if not svc_local.is_file() or not tim_local.is_file():
            sys.exit("missing deploy/nfl-edge-live.service or .timer")
...
# 1) Remove ONLY NFL EDGE lines from root crontab (leave everything else)
...
grep -v 'nfl-edge' /tmp/cron_before_nba \\
  | grep -v 'run_nfl_edge.py' \\
  | grep -v 'run_if_et.sh' \\
  | grep -v 'NFL EDGE' \\
...
if crontab -l 2>/dev/null | grep -E 'run_nfl_edge|run_if_et|NFL EDGE' >/dev/null; then
...
echo {shlex.quote(svc_b64)} | base64 -d > /etc/systemd/system/nfl-edge-live.service
echo {shlex.quote(tim_b64)} | base64 -d > /etc/systemd/system/nfl-edge-live.timer
systemctl daemon-reload
# must show America/New_York calendars
grep -E '11:35:00 America/New_York|18:50:00 America/New_York' \\
  /etc/systemd/system/nfl-edge-live.timer
if grep -q '17:45:00' /etc/systemd/system/nfl-edge-live.timer; then
  echo ABORT: 17:45 still present
  exit 46
fi
for slot in '11:35:00' '18:50:00'; do
  grep -q "$slot America/New_York" /etc/systemd/system/nfl-edge-live.timer || {{
    echo "ABORT: missing slot $slot"; exit 46;
  }}
done
# Operator stop: /root/nfl-edge/STOPPED keeps schedule disabled
if [ -f {VPS_REMOTE_DIR}/STOPPED ]; then
  systemctl disable --now nfl-edge-live.timer 2>/dev/null || true
  systemctl stop nfl-edge-live.service 2>/dev/null || true
  echo TIMER_STOPPED
  systemctl is-enabled nfl-edge-live.timer 2>&1 || true
else
  systemctl enable --now nfl-edge-live.timer
  systemctl restart nfl-edge-live.timer
  echo TIMER_OK
  systemctl is-enabled nfl-edge-live.timer
  systemctl list-timers nfl-edge-live.timer --no-pager
fi
echo SCHEDULE_OK
"""
...
def verify_vps(env: dict[str, str]) -> None:
    ...
    r = ssh(
        ...
        f"systemctl is-enabled nfl-edge-live.timer 2>/dev/null || true; "
        f"systemctl is-active nfl-edge-live.timer 2>/dev/null || true; "
        f"test -f {VPS_REMOTE_DIR}/STOPPED && echo STOPPED_FLAG || echo RUNNING_FLAG; "
        f"grep -E 'America/New_York' /etc/systemd/system/nfl-edge-live.timer 2>/dev/null; "
        f"crontab -l 2>/dev/null | grep -E 'run_nba|NFL EDGE' || echo NO_NBA_IN_ROOT_CRONTAB; "
    ...
    checks = {
        ...
        "timer Sun 11:35 ET": "11:35:00 America/New_York" in blob,
        "timer Thu/Mon 18:50 ET": "18:50:00 America/New_York" in blob,
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Sync NFL EDGE: Git ↔ PC ↔ VPS")
    ...
    ap.add_argument(
        "--env",
        default=os.environ.get("NFL_EDGE_ENV", str(DEFAULT_ENV)),
    ...
    print("\n=== TESTS ===")
    tr = run(
        [sys.executable, "-m", "pytest", "nfl_edge/tests", "-q", "--tb=short"],
        check=False,
    )
    if tr.returncode != 0:
    ...
    pipe_src = (ROOT / "nfl_edge" / "pipeline.py").read_text(encoding="utf-8")
    brain_src = (ROOT / "nfl_edge" / "brain.py").read_text(encoding="utf-8")
